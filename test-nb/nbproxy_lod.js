...
const LOGDIR = process.env.LOGDIR ? (path.isAbsolute(process.env.LOGDIR)?process.env.LOGDIR:path.join(__dirname,process.env.LOGDIR)) : path.join(__dirname, 'logs_nb');
...
NB.pressure.configure({ highBytes: 32e6, lowBytes: 8e6, minHoldMs: 2000 });
...
tsServer.on('connection', sock => {
    ...
    // LOD request: client -> server only (not broadcast)
    sock.on('/req', body => {
        if (serverSock && serverSock.connected) serverSock.io('/req', body);
    });
...
setInterval(() => {
    ...
    mStream.write(JSON.stringify({
        ...
        held: NB.pressure ? (NB.pressure.state().held ? 1 : 0) : 0,
        agg: NB.pressure ? NB.pressure.state().aggBytes : 0
    }) + '\n');
    eld.reset();
}, 250);
