...
const LOGDIR = path.join(__dirname, process.env.LOGDIR || 'logs');
const SEED = 42;
...
function start(name, args, env) {
    const p = spawn('node', args, {
        cwd: __dirname,
        env: { ...process.env, LIB: process.env.LIB||'', LOGDIR, ...env },
...
setTimeout(() => start('proxy', ['uproxy.js'], { LIB: process.env.LIB, LOGDIR }), 500);
setTimeout(() => start('server', ['userver.js'], { SEED: String(SEED), T0: String(T0), WINDOW_MS: String(WINDOW_MS), PORT: '9199', NMSG: process.env.NMSG || '200', FILE_MIN_MB: process.env.FILE_MIN_MB || '5', FILE_MAX_MB: process.env.FILE_MAX_MB || '100' }), 1500);
...
setTimeout(() => {
    ...
    start('clientsA', ['uclient.js'], { SPEC: aSpec, KIND: 'A', SEED: String(SEED), T0: String(T0), WINDOW_MS: String(WINDOW_MS), DEADLINE: String(DEADLINE), NMSG: process.env.NMSG || '200' });
    for (const c of clientSpecs.filter(c => c.kind === 'B')) {
        start(`client${c.name}`, ['uclient.js'], { SPEC: `${c.name}:${c.port}`, KIND: 'B', SEED: String(SEED), T0: String(T0), WINDOW_MS: String(WINDOW_MS), DEADLINE: String(DEADLINE), NMSG: process.env.NMSG || '200' });
    }
}, 3000);
